/**
 * Quantum Hello World — Wired to real P31 stack (PWA)
 * Uses @p31/node-zero, @p31/love-ledger, @p31/game-engine.
 */
...
const CONVERSATION_LINES: { speaker: "P" | "U"; text: string | null }[] = [
...
function formatLove(n: number): string {
  return n.toFixed(1);
}

type DisplayTx = { type: string; amount: number };

function mapTxToDisplay(tx: { type: string; amount: number; meta?: Record<string, unknown> }): DisplayTx {
...
function WalletDisplay({ total }: { total: number }) {
  const sov = total / 2;
  const perf = total / 2;
  return (
    <div
...
export function QuantumHelloWorld() {
  ...
  const [covenantAccepted, setCovenantAccepted] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (phase === 1 && messages.length === 0) {
      setTimeout(() => {
        const first = CONVERSATION_LINES[0];
        if (first?.text) setMessages([{ speaker: "P", text: first.text }]);
        setConvStep(1);
        setCoherence(0.08);
      }, 800);
    }
  ...
  const sendMessage = useCallback(() => {
    ...
    const newMessages = [...messages, { speaker: "U" as const, text: userInput }];
    ...
    if (step < CONVERSATION_LINES.length) {
      ...
      if (nextLine?.speaker === "P") addNext(1000);
      else setConvStep(step);
    }
  ...
  const acceptCovenant = useCallback(() => {
    const current = COVENANT_VALUES[covenantIndex];
    if (!current) return;
    const accepted = [...covenantAccepted, current.key];
  ...
  const displayTx: DisplayTx[] = ledger
    ? ledger.transactions
        .map((tx: { type: string; amount: number; meta?: Record<string, unknown> }) =>
          mapTxToDisplay({ type: tx.type, amount: tx.amount, meta: tx.meta })
        )
        .filter((t: DisplayTx) => ["GENESIS", "RESONANCE", "FIRST_STRUCTURE"].includes(t.type))
    : [];
    ...
    >
      ...
      {phaseName === "CONVERSE" && (
        <div style={{ maxWidth: 520, width: "100%" }}>
          ...
          <div
            ref={chatRef}
            style={{ marginTop: 16, maxHeight: 320, overflowY: "auto", padding: "12px 0", scrollBehavior: "smooth" }}
          >
            {messages.map((msg, i) => (
              <div
                key={i}
      ...
      {phaseName === "COVENANT" && covenantIndex >= 0 && (
        <div style={{ maxWidth: 520, width: "100%", textAlign: "center" }}>
          ...
          <button
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLButtonElement).style.background = "#31ffa320";
            }}
            onMouseLeave={(e) => {
        ...
        <div style={{ maxWidth: 560, width: "100%", animation: "slideup 0.6s ease" }}>
            ...
            {displayTx.map((tx, i) => (
              <div
                ...
                style={{
                  ...
                  justifyContent: "space-between",
                  padding: "6px 0",
                  borderBottom: i < displayTx.length - 1 ? "1px solid #111" : "none",
                  fontFamily: "'DM Mono', monospace",
                  fontSize: 12,
          ...
          {CHALLENGE_0 && (
            ...
            >
              ...
              >
                ✓ CHALLENGE #0 COMPLETE — {(CHALLENGE_0 as { title?: string }).title?.toUpperCase() ?? "THE RESONANCE"}
              </div>
              <div style={{ fontSize: 13, color: "#888", fontStyle: "italic", lineHeight: 1.6 }}>
                &quot;{(CHALLENGE_0 as { fullerPrinciple?: string }).fullerPrinciple ?? "Unity is plural and at minimum two."}&quot;
              </div>
            </div>
          )}
          {structure0 && rigidity && (
            ...
            >
              ...
              >
                <span style={{ color: "#888" }}>
                  Vertices: {rigidity.vertices} · Edges: {rigidity.edges}
                </span>
              ...
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#555", marginTop: 4 }}>
                Maxwell: E ≥ 3V − 6 → {rigidity.edges} ≥ {rigidity.maxwellThreshold ?? 6} ✓{" "}
                {rigidity.isRigid ? "RIGID" : "FLEX"}
              </div>
            </div>
          )}
